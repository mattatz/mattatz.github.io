...
void main() {
    ...
    if(distance(texcoord, mouse) < radius) {
        ...
        int values[6];
        values[0] = boolean(vNEESE.r);
        values[1] = boolean(vNEESE.g);
        values[2] = boolean(vNEESE.b);
        values[3] = boolean(vSWWNW.r);
        values[4] = boolean(vSWWNW.g);
        values[5] = boolean(vSWWNW.b);

        int ne = collide(values, irandom(texcoord, t + 13.));

        shift(values);
        int e = collide(values, irandom(texcoord * 23., t - 9.));

        shift(values);
        int se = collide(values, irandom(texcoord * 7., t + 5.));

        result.rgb = vec3(ne + int(texture2D(velocityNEESE, texcoord).r), e + int(texture2D(velocityNEESE, texcoord).g), se + int(texture2D(velocityNEESE, texcoord).b));
    }
